...
public class WordNet
{
     ...
     * </TABLE>
     */
    private static ArrayList[] allSenseLists = new ArrayList[ WN_SENSE_LISTS ];
    ...
    static void preScan( File[] xmlFiles )
    {
        ...
        try
        {
            ...
            if( Generator.flVerbose && !Generator.flQuiet ) {
                System.out.print( "         [" );
            }

            // Set the first goal to reach as 2.5% of the total number of files.
            double goal = 0.025;
            ...
            for( int x = 0; x < xmlFiles.length; x++ )
            {
                ...
                if( Generator.flVerbose && !Generator.flQuiet )
                {
                    double pct = ( double ) x / xmlFiles.length;
                ...
                for( int m = 0; m < members.getLength(); m++ )
                {
                    ...
                    for( int s = 0; s < thisVerbsSenses.length; s++ )
                    {
                        ...
                        {
                            errors.add( "ERROR: [" + xmlFiles[ x ].getName() + "] The sense key \"" + sense + "\" is invalid." );
                            continue;
                        }
    ...
    static void loadSenseNumbers( File senseIndexFile )
    {
        try
        {
            ...
            // Flag used to discontinue processing when all sense keys
            // have been visited (all the VerbNet sense keys have had their
            // sense numbers looked up).
            boolean notDone = true;

            // Process each line in the file, checking to see if it
            // corresponds to a key that is required by the VerbNet system.
            while( ( line = senseIndex.readLine() ) != null && notDone )
            {
     ...
     * @see uvi.Sweeper#printMembers()
     */
    static String getSenseNumber( String curClass, String key )
    {
        ...
        {
            System.err.println( "ERROR: [" + curClass + "] The sense key \"" + key + "\" is invalid." );
            return "?";
        }
     ...
     */
    private static class WordNetSense
    {
