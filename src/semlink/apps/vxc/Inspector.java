 ...
 * this class and the {@link vn.Sweeper} class use to customize the output.  There are five
 * operators added to the original Inspector to implement the VxC application: -C, -M, -F, A, and -S.  Look
 * at the full usage for further details. The general usage is: <BR>
 * <PRE>&nbsp;&nbsp;&nbsp;java vn.Inspector [flags] &lt;x> [-V &lt;v>] [-O &lt;o>] -C &lt;c> -M &lt;m> -F &lt;f> [-A &lt;a>] [-S &lt;s>]</PRE>
 * See the usage note (<CODE>java vn.Inspector -?</CODE>) for full details.
 * <BR><BR>Although the Inspector has two basic output modes: normal output mode, and class
 * hierarchy mode, VxC is almost always run without either the -V opreator or -c flag supplied.
 * VxC relies completely on the event system and its own external file manipulation.
 * The -C operator specifies the Cyc input file containing
 * all the <CODE>verbSemTrans</CODE> semantic predicates.  The -M operator specifies the
 ...
 * <BR><BR>
 * There are  two optional VxC operators, -A and -S.  The -A operator specifies the constraints(s) to use
 * when deciding which matches are valid and which should be discarded.  The -S operator is similar to [flags], and just passes
 * VxC-specific flags onto the underlying mapping system.
 * <BR><BR>
 ...
 * order to avoid processing unwanted files. VxC is almost never run with the -O operator.
 * It needs to look over matches from all the files.  However, there is a {@link vn.Cyc#flManualClassesOnly}
 ...
 *    java vn.Inspector verbnet-xml/ -Vfcm -Och > output</PRE>
 ...
 * <BR><BR>For debugging purposes, the {@link vn.Cyc#flShowDiscards} flag (set with -Sd) option forces a line of text to be printed
 * to <B>stderr</B> for each naive match that was discarded (after having the constraints specified
 ...
 *
 * csh/tcsh:
 *    ( java vn.Inspector verbnet-xml/ [...] -Sd > temp ) >& reasons-for-discard</PRE>
 *
 * Above, <CODE>[...]</CODE> represents all the other operators and flags you would supply for the execution (i.e. -C, -M, -F).
 * For csh/tcsh shells, the <CODE>temp</CODE> file is required to properly extract just the
 * errors from the program into <CODE>reasons-for-discard</CODE>.
 * <BR><BR>The only changes made to this class for the VxC extension pertained to adding the new command-line arguments.
 * The usage/help message was modified and {@link vn.Inspector#analyzeArguments(String[])} was enhanced to
 * validate them and pass them on to the {@link vn.Cyc} class.
 * <BR><BR>
...
public class Inspector
{
   ...
   /**
    * All operators used on the command line.  There are two standard
    * Inspector operators (-V and -O) and five VxC application-specific
    * ones (-C, -M, -F, -A and -S).  See usage for more details.
    *
    * @see vn.Inspector#analyzeArguments(String[])
    */
   private static final String ALL_OPER_CHARS = "VOCMFAS";
   ...
   /**
    * Sorts the already loaded XML files array {@link vn.Inspector#xmlFiles}.
   ...
   public static void main( String[] args )
   {
      ...
      catch( InvalidCommandLineArgumentException iclae )
      {
         ...
         eprintln( "" );
         eprintln( "Usage: java vn.Inspector [flags] <x> [-V <v>] [-O <o>] -C <c> -M <m> -F <f> [-A <a>] [-S <s>]" );
   ...
   {
      println( "Usage: java vn.Inspector [flags] <x> [-V <v>] [-O <o>] -C <c> -M <m> -F <f> [-A <a>] [-S <s>]" );
      println( "" );
      println( "NOTE: Command-line elements can occur in any order.  Any element between" );
      println( "      [ ] is optional.  -V, -O, etc. are called \"operators\".  Space between" );
      ...
      println( "" );
      println( "<c>: \"Cyc Rule File\"" );
      println( "        A properly formatted file containing all of the Cyc verbSemTrans" );
      println( "        rules.  The format of this file is the same format as what is produced" );
      println( "        when the 'predicate extent' option is selected for the verbSemTrans" );
      println( "        predicate in the Cyc Knowledge Browser." );
      println( "" );
      println( "<m>: \"Manual Mapping File\"" );
      println( "        The manual mapping XML file against which to compare Cyc matches." );
      println( "        Results are printed to stdout after the VerbNet information." );
      println( "" );
      println( "<f>: \"Match Output File\"" );
      println( "        The file to create in a standard XML format representing all the matches" );
      println( "        made by the automatic matching algorithm." );
      println( "" );
      println( "<a>: \"Match Constraints\"" );
      println( "        The constraint(s) to use for deciding if a match exists between a VerbNet" );
      println( "        verb-frame pair and a Cyc verbSemTrans rule.  Each is represented by a" );
      println( "        single character.  Can mix-and-match as in this example: '-Atp'.  All" );
      println( "        constraints besides 0 include n automatically." );
      println( "            0 - Debug Mode: Matches nothing regardless, only used alone" );
      println( "            n - Naive Matching: Matches on lemma (default)" );
      println( "                 (VerbNet Member and Cyc *-TheWord must be equal)" );
      println( "            p - VN must have preposition if Cyc PPCompFrameFn" );
      println( "            t - Transitivity congruence" );
      println( "            i - Infinitive/gerund congruence" );
      println( "            j - Adjective congruence" );
      println( "            f - Cyc fromLocation implies VN Source" );
      println( "            d - Cyc doneBy/performedBy implies VN Agent" );
      println( "            m - Cyc MiddleVoiceFrame implies no VN Agent" );
      println( "            a - enable all constraints (nptijfdm)" );
      println( "        If this operator is not supplied, -An is assumed.  More information" );
      println( "        concerning the constraints can be found in the Javadoc documentation" );
      println( "        for the Cyc and Matcher classes." );
      println( "" );
      println( "<s>: \"VxC Application Flags\"" );
      println( "        Additional flags for the VxC application.  These are:" );
      println( "            c - Print class match counts" );
      println( "            d - Print a line to stderr for each discarded match" );
      println( "            m - Only look for matches in classes that are covered by the manual" );
      println( "                mapping file; ignore all other classes" );
      println( "" );
    ...
    * all thosme methods which directly call {@link vn.Inspector#incrementTagCounter(String)}.
             ...
             UserWantsKey
   {
         ...
         if( args[ a ].startsWith( "-" ) )
         {
            ...
            if( ALL_OPER_CHARS.indexOf( key ) != -1 )
            {
               ...
               switch( key )
               {
                  ...
                  // Arguments for the VxC application.
                  case 'C': Cyc.cycFile           = new File( buddy ); break;
                  case 'M': Cyc.manFile           = new File( buddy ); break;
                  case 'F': Cyc.matchFile         = new File( buddy ); break;
                  case 'A': Cyc.matchConstraints += buddy;             break;
                  case 'S':

                     // Cycle through all the letters in the VxC flag.
                     for( int b = 0; b < buddy.length(); b++ )
                     {
                        char flag = buddy.charAt( b );

                        // Append this flag to a string of all supplied flags.
                        if( Cyc.flags == null ) Cyc.flags = "";
                        Cyc.flags += flag;

                        // Set the appropriate boolean flag variable.
                        switch( flag )
                        {
                           case 'c': Cyc.flShowClassMatchCounts = true; break;
                           case 'd': Cyc.flShowDiscards         = true; break;
                           case 'm': Cyc.flManualClassesOnly    = true; break;

                           // Catch invalid flags.
                           default:
                              throw new InvalidCommandLineArgumentException( "Invalid VxC flag (" + flag + ")." );
      ...
      // Clean the VxC flags variable.
      Cyc.flags = wrapString( Cyc.flags );

      // If the VxC match constraints operator was not specified, set the default of 'n'.
      if( Cyc.matchConstraints.equals( "" ) )
         Cyc.matchConstraints = "n";

      // Else validate the match constraints supplied.
      else
      {
         boolean allConstr = false;

         // Scan through all the letters in the match constraints and
         // make sure each one is in the 'all' constant.
         for( int ma = 0; ma < Cyc.matchConstraints.length(); ma++ )
         {
            char constr = Cyc.matchConstraints.charAt( ma );

            if( Cyc.ALL_MATCH_CONSTRS.indexOf( constr ) == -1 )
               throw new InvalidCommandLineArgumentException( "Invalid match constraint (" + constr + ")." );

            if( constr == 'a' )
               allConstr = true;
         }

         // Validate the null constraint's use.
         if( Cyc.matchConstraints.indexOf( "0" ) != -1 && Cyc.matchConstraints.length() != 1 )
            throw new InvalidCommandLineArgumentException( "Cannot use match constraint 0 with any other." );

         // If an 'a' was found, use all the constraints from the 'all' constant
         // without the '0' or 'a'.
         if( allConstr )
            Cyc.matchConstraints = Cyc.ALL_MATCH_CONSTRS.replaceAll( "[0a]", "" );

         // Make sure that the naive match constraint is applied first.
         // The naive constraint must always be applied first.
         if( !Cyc.matchConstraints.equals( "0" ) && Cyc.matchConstraints.charAt( 0 ) != 'n' )
            Cyc.matchConstraints = 'n' + Cyc.matchConstraints;
      }

      // Make sure the VxC Cyc input file is a valid, readable file.
      if( Cyc.cycFile != null )
      {
         if( !Cyc.cycFile.exists() )   throw new InvalidPathException( "Cyc input file path does not exist." );
         if( !Cyc.cycFile.isFile() )   throw new InvalidPathException( "Cyc input file path is not a file."  );
         if( !Cyc.cycFile.canRead() )  throw new InvalidPathException( "Cyc input file not readable."        );

         // Set up the path to the compressed file.
         Cyc.cycFileCmp = new File( Cyc.cycFile.getName() + ".vxc.compressed" );
      }
      else
         throw new InvalidCommandLineArgumentException( "No Cyc input file specified." );

      // Make sure the VxC manual mapping input file is a valid, readable file.
      if( Cyc.manFile != null )
      {
         if( !Cyc.manFile.exists() )   throw new InvalidPathException( "Manual mapping input file path does not exist." );
         if( !Cyc.manFile.isFile() )   throw new InvalidPathException( "Manual mapping input file path is not a file."  );
         if( !Cyc.manFile.canRead() )  throw new InvalidPathException( "Manual mapping input file not readable."        );
      }
      else
         throw new InvalidCommandLineArgumentException( "No manual mapping input file specified." );

      // Make sure the VxC match output file is a valid, writable file.
      if( Cyc.matchFile != null )
      {
         if( Cyc.matchFile.exists() && !Cyc.matchFile.isFile() )
            throw new InvalidPathException( "Match output path is not a file." );

         if( Cyc.matchFile.exists() && Cyc.matchFile.isFile() && !Cyc.matchFile.canWrite() )
            throw new InvalidPathException( "Match output file is not writable."  );
      }
      else
         throw new InvalidCommandLineArgumentException( "No match output file specified." );
